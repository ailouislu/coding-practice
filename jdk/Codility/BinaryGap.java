...
//        int inputNumber = 15;
//        int inputNumber = 32;
        System.out.println(lengthOfBinaryGapLambda(inputNumber));
...
//        bufferedReader.close();
    ...
    public static int lengthOfBinaryGapLambda(int inputNumber) {
        String binary = Integer.toBinaryString(inputNumber);
        final int[] count = {0};
        final int[] tmpCount = {0};
        binary.chars().forEach(c -> {
            if (c == '0') {
                ...
                if (binary.charAt(binary.indexOf(c) - 1) == '1') {
                    tmpCount[0]++;
                    return;
                }
            }
            if (tmpCount[0] > 0 && tmpCount[0] > count[0]) {
                count[0] = tmpCount[0];
            }
            tmpCount[0] = 0;
        });
